...
This software expects a layout looking roughly like the following
 mainfolder/
     nzb/             -- nzb processing, put ziplick in here
		 ...
		 self.current/        -- don't mess with this, its the self.current nzb file being worked on
		 self.temp/           -- self.temp space for nzbget (set this in your nzbget rc file)
...
no effect as it will start on the self.current nzbfile again when spawned.
...
class Ziplick(Thread):
        ...
        def run(self):
                self.queued_nzbs = []
                self.current_nzbs = [x for x in os.listdir(self.current) if re.search(r'\.nzb$',x)]
                
                while 1:
                	# See if we're resuming a nzb fetch
                	if not self.current_nzbs:
                
                		# Refresh our self.queue and append the new nzb's, 
                		new_nzbs = [x for x in os.listdir(self.queue) if x not in self.queued_nzbs \
                                            and re.search(r'\.nzb$',x)]
                		...
                		os.rename(self.queue+nzbfilename,self.queue+newname)
                		nzbfilename = newname
                
                		# nzbfile will always be a absolute filename 
                		nzbfile = self.queue + nzbfilename
                		os.spawnlp(os.P_WAIT, 'mv', 'mv', nzbfile, self.current)
                	else:
                		nzbfilename = self.current_nzbs[0]
                		del self.current_nzbs[0]
                	nzbfile = self.current + nzbfilename
                
                	# Run nzbget to fetch the self.current nzbfile
                	result = os.spawnlp(os.P_WAIT, 'nzbget', 'nzbget', nzbfile)
                
                	# Make our new directory, minus the .nzb
                	newdir = self.dest + nzbfilename
                	newdir = re.sub(r'\.nzb$',r'',newdir)
                	newdir = re.sub(r'msgid_.*?_',r'',newdir)
                	...
                	# Move our nzb contents to their new location, clear out the self.temp dir
                        print "working: " + self.working + " newdir: " + newdir
                	os.rename(self.working,newdir)
