...
class NewsShelf:
    ...
    def groupRequest(self, group):
        ...
        try:
            ...
            return defer.succeed((group, num, high, low, flags))
            """
        # FIXME: For testing -Rubberneck changes
        """
        import time
        time.sleep(2)
        return defer.fail(NewsServerError("No such group: " + group))
        """
        return defer.succeed((group, 1, 2, 3, ''))
    ...
    def bodyRequest(self, group, index, id = None):
        ...
        try:
            #a = self.dbm['groups'][group].articles[index]
            body = self.dbm['BODIES'][index]
        except KeyError:
            return defer.fail(NewsServerError("No such group: " + group))
        else:
            return defer.succeed((index, a.getHeader('Message-ID'), StringIO.StringIO(body)))
            """
...
class NNTPFactory(protocol.ServerFactory):
    ...
    def buildProtocol(self, connection):
        ...
        p.factory = self
        return p
ns = NewsShelf(None, '/beans/outgoing/dirdbm')
nf = NNTPFactory(ns)
reactor.listenTCP(31337, nf)
reactor.run()
